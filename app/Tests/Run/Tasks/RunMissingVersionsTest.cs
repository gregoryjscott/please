...
namespace Tests.Run.Tasks
{
    ...
    public class RunMissingVersionsTest
    {
        ...
        public void should_run_sql_scripts_if_no_versions_exist()
        {
            ...
            runMissingVersions.In.InstalledVersions = new Please.Run.Models.Version[0];
            runMissingVersions.In.Scripts = RunSomethingTest.TestScripts;
        ...
        public void should_not_run_any_sql_scripts_if_all_versions_exist()
        {
            // Arrange
            var runMissingVersions = Task.New<RunMissingVersions>();
            runMissingVersions.In.InstalledVersions = RunSomethingTest.TestVersions;
            runMissingVersions.In.Scripts = RunSomethingTest.TestScripts;
            runMissingVersions.RunScripts = Fake.Task<RunScripts>();
            runMissingVersions.InsertInstalledVersion = Fake.Task<InsertInstalledVersion>();
        ...
        public void should_run_all_sql_scripts_for_a_missing_version()
        {
            ...
            var runMissingVersions = Task.New<RunMissingVersions>();
            runMissingVersions.In.InstalledVersions = RunSomethingTest.TestVersions.Where(version => version.Id != "002").ToArray();
            runMissingVersions.In.Scripts = RunSomethingTest.TestScripts;
            runMissingVersions.RunScripts = Fake.Task<RunScripts>(
        ...
        public void should_run_sql_scripts_in_alphabetical_order()
        {
            ...
            var runMissingVersions = Task.New<RunMissingVersions>();
            runMissingVersions.In.InstalledVersions = new Please.Run.Models.Version[0];
            runMissingVersions.In.Scripts = RunSomethingTest.TestScripts.Reverse().ToArray();
            runMissingVersions.RunScripts = Fake.Task<RunScripts>(
            ...
            // Assert
            Assert.That(runOrder[0], Is.EqualTo(RunSomethingTest.TestScripts[0].FileName));
            Assert.That(runOrder[1], Is.EqualTo(RunSomethingTest.TestScripts[1].FileName));
            Assert.That(runOrder[2], Is.EqualTo(RunSomethingTest.TestScripts[2].FileName));
            Assert.That(runOrder[3], Is.EqualTo(RunSomethingTest.TestScripts[3].FileName));
        }
    }
}
