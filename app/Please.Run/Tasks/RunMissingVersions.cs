        ...
        public override void Execute()
        {
            ...
            foreach (var versionId in allVersionIds)
            {
                if (In.InstalledVersions.All(installed => installed.Id != versionId))
                {
                    ...
                    System.Console.WriteLine("{0} not installed - running scripts.", missingVersionId);

                    var sqlScriptsForMissingVersion = In.Scripts
                        .Where(m => m.VersionId == missingVersionId)
                        .OrderBy(m => m.FileName);

                    System.Console.WriteLine("{0} scripts were found for version {1}.",
                ...
                {
                    System.Console.WriteLine("{0} already installed.", versionId);
                }
            }
        }
    }
